#!/usr/bin/env python3
"""
Unsloth GPT-OSS-20B Training Script - Following Official Unsloth Patterns
Direct Python configuration with command-line arguments for flexibility
"""
...
# Configuration profiles (Unsloth way - Python dicts, not YAML)
PROFILES = {
    "quick_test": {
        "max_steps": 30,
        "dataset_size": 100,
...
def main():
    parser = argparse.ArgumentParser(description='Unsloth GPT-OSS-20B Training')
    ...
    print("=" * 60)
    print(f"UNSLOTH TRAINING - Profile: {args.profile}")
    ...
    # ============================================================
    # STEP 1: Load Pre-Quantized Model (Official Unsloth Way)
    # ============================================================
    max_seq_length = 2048 if r >= 16 else 1024  # Longer context for higher rank
    ...
    # ============================================================
    lora_alpha = r * 2  # Unsloth recommendation: 2x rank

    # Build configuration based on rank
    peft_config = {
        "r": r,
    ...
    if r >= 16:
        peft_config["use_rslora"] = True  # Rank-stabilized
        peft_config["loftq_config"] = {   # Better initialization
            "loftq_bits": 4,
            "loftq_iter": 1,
        }
    ...
    if r >= 16:
        print("  Using RSLoRA and LoftQ for stability")
    ...
    # ============================================================
    # STEP 4: Training Configuration (SFTConfig - Unsloth way)
    # ============================================================

    # Determine warmup steps based on scheduler
    ...
    trainer = SFTTrainer(
        ...
        args=SFTConfig(
            ...
            output_dir=f"{args.output_dir}_{args.profile}",
            save_strategy="steps",
            ...
            seed=3407,
            max_seq_length=max_seq_length,
            packing=False,  # Keep disabled for stability
        ),
    )

    # ============================================================
    # STEP 5: Train on Completions Only (GPT-OSS feature)
    # ============================================================
...
if __name__ == "__main__":
    # Show available profiles if no args
    import sys
    if len(sys.argv) == 1:
        ...
        print("\nExample usage:")
        print("  python train.py --profile quick_test")
        print("  python train.py --profile standard --scheduler cosine")
        print("  python train.py --profile max_quality --r 16 --batch_size 6")
